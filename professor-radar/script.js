...
async function load(){
  try{
    ...
    document.querySelector('#positions').innerHTML=rows.length?rows.map(p=>`<tr><td>${esc(p.university)}</td><td><strong>${esc(p.title)}</strong>${p.department?`<br><span class="muted">${esc(p.department)}</span>`:''}</td><td>${date(p.deadline)}</td><td>${p.fit_score?`${esc(p.fit_score)}/5`:'Verified'}</td><td>${esc(p.why_now||'Direct rank and technical-area match.')}</td><td><a href="${esc(p.link)}" target="_blank" rel="noopener">Official notice →</a></td></tr>`).join(''):'<tr><td colspan="6" class="muted">No matching open positions found in the latest scan.</td></tr>';
  ...
  }catch(e){document.querySelector('#positions').innerHTML='<tr><td colspan="6" class="muted">Radar data is temporarily unavailable.</td></tr>'}
}
