...
When `funding_calls.xlsx` changes, run `import_funding_sources.py` and preserve every row in `FUNDING_SOURCES.md`, separating non-past calls from historical ones. The workbook has no URLs; never invent them or present inferred funding bodies as verified.
